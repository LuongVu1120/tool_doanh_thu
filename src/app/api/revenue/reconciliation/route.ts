...
import { buildRevenueReport, reconcileWithGoldenApril2026 } from '@/lib/revenue/report-engine'
...
export async function GET(request: NextRequest) {
  try {
    ...
    const period = searchParams.get('period') || GOLDEN_APRIL_2026_PERIOD
    if (period !== GOLDEN_APRIL_2026_PERIOD) {
      return NextResponse.json({ error: `Golden PDF hiện chỉ có kỳ ${GOLDEN_APRIL_2026_PERIOD}` }, { status: 400 })
    }
    ...
    return NextResponse.json({
      report,
      reconciliation: reconcileWithGoldenApril2026(report),
    })
  } catch (error) {
    return NextResponse.json(
